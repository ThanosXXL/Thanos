...
## Baustelle Tagesreport (separates Projekt)

Der Baustelle-Tagesreport ist **keine** Ansicht dieser App mehr, sondern eine komplett eigenständige
Desktop-Anwendung im Ordner [`baustelle-tagesreport/`](baustelle-tagesreport/) – eigene Codebasis, eigener
Electron-Prozess, eigene Datenablage, keine Verlinkung zwischen beiden Apps. Details siehe die
[README](baustelle-tagesreport/README.md) dort.
...
`renderer/index.html` funktioniert auch direkt in jedem Browser (z. B. über GitHub Pages oder einen
beliebigen Webserver) und lässt sich dank `manifest.json` + Service Worker als Web-App installieren
("Zum Home-Bildschirm hinzufügen" auf iOS/Android).
