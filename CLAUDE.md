...
A third, also unrelated project lives in `radaralarm/`: **RadarAlarm** is a separate Electron desktop app — a demo-scoped construction-documentation / defect-management ("Mängelmanagement") / communication tool for building and real-estate projects (floor plans with clickable defect pins, defect cards with photo/status/priority/trade/due date, and a per-defect comment thread). It has its own `package.json`, `main.js`, `preload.js`, and `renderer/`, follows the same Electron hardening pattern as the Dozenten Dashboard (CSP, sandboxed `contextIsolation`, no direct Node access from the renderer), and persists to its own local JSON file — nothing is shared with the Dozenten Dashboard's data or code. It intentionally enforces demo limits (max projects / max defects per project, checked in the main process, not just the UI) since it has no backend — there is no license/payment or cloud-sync infrastructure behind it. Run it with `npm start` from inside `radaralarm/` (after its own `npm install`). No CI/release workflow is wired up for it yet.
