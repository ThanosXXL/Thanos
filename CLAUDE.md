...
A fourth project, `akademie/`, is unrelated to all of the above: it's a collection of Python/ReportLab scripts that generate M&C Akademie course-material PDFs (course descriptions, participant/instructor handbooks, slide decks, exams) rather than an Electron app. See `akademie/CLAUDE.md` for the full layout, the house logo/font assets and their non-negotiable usage rules (logo only on page 1, Work Sans not Helvetica, etc.), and the shared `akademie_style.py` layout module every build script should reuse.
