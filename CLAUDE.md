...
A fourth project, `tattoo-studio-webshop/`, is a standalone marketing/booking website for a fictional tattoo studio ("Inkarnation Tattoo Studio") — plain static HTML/CSS/vanilla JS with no build step, no framework, and no dependency on Electron or Node at runtime. It is a public-facing single-page site, not a desktop app: open `tattoo-studio-webshop/index.html` directly in a browser, or serve the folder with any static file server. `index.html` holds the whole page (collapsible left sidebar nav, hero, artist roster, image gallery, services, reviews, contact/booking), `css/style.css` carries the dark glossy/3D visual theme (glassmorphism cards, mouse-driven tilt, glossy buttons), and `js/main.js` is a single IIFE handling sidebar toggling, scroll-spy navigation, the booking modal, and the mailto-based appointment form (with a `tel:` link as the phone alternative). All artist portraits and gallery pieces under `assets/images/` are hand-authored SVG illustrations (not photos). It shares no dependencies, build config, or CI with the Dozenten Dashboard, `omniroute/`, or `it-schulung/`.

`tattoo-studio-webshop/branding/` holds durable marketing/demo assets (a screen-recorded site walkthrough muxed with procedurally-synthesized lounge music, in both MP4/H.264 and WebM/VP9 for codec coverage, plus the music alone as MP3) and two standalone single-file HTML players with the media embedded as base64 data URIs, each with a working download button. Those download buttons only function when the HTML file is opened directly in a browser (local file or static hosting) — a sandboxed preview iframe blocks script- and anchor-driven downloads. See `tattoo-studio-webshop/branding/README.md`.
