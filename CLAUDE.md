...
- **`docs/`** — static, standalone HTML pages (not part of the Electron app bundle), all built around **`download.html`** as the single landing/download hub.
  ...
  **Narrower, later exception:** the hero's primary "⬇ 1-Klick-Downloader-App starten" button and the "Direkt-Download ohne App" Windows/macOS/Linux links were removed from `download.html`'s hero at the user's explicit request — those installer download links are now only relayed directly in chat, not featured on the page itself. Same pattern as the Demo-Video (still on the page) and the PowerShell card (removed, see below): the user asks per-item whether something belongs on the page or only in chat: don't assume, and don't silently re-add the hero button/os-links unless asked.
