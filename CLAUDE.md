...
- **`docs/`** — static, standalone HTML pages (not part of the Electron app bundle): `download.html` (styled download/install landing page, links to everything below — currently features the Inventar-only demo, not the combined one), `handbuch.html` / `handbuch-inventar.html` / `admin-handbuch.html` (user, Inventar-specific, and admin/technical manuals, all with screenshots from `docs/images/`), and `demo.html` (a browser-only copy of the full UI — both modes, mode switch included — that runs without Electron via a `localStorage`-backed `dashboardAPI` shim in `demo-shim.js`, so it reuses `renderer/renderer.js` unmodified). `demo-inventar.html` + `demo-inventar-shim.js` are an Inventar-only variant: the mode-switch UI stays in the DOM (so `renderer.js`'s `getElementById` calls don't throw) but is hidden via CSS, and an inline script clicks `modeInventarBtn` synchronously right after `renderer.js` loads — before the first async render — so the Dozenten dashboard is never visible, not even briefly; it also overrides `document.title` since `renderer.js`'s demo-ribbon logic unconditionally sets it to a Dozenten-branded string. `demo-standalone.html` / `demo-inventar-standalone.html` are generated, fully self-contained versions (CSS/JS inlined) meant to be downloaded and opened offline; regenerate both with `npm run build:demo-standalone` (runs `docs/build-standalone-demo.js`) whenever `renderer/style.css`, `renderer/renderer.js`, `docs/demo-shim.js`, or `docs/demo-inventar-shim.js` change — they are not rebuilt automatically. Every demo HTML file must stay in sync with `renderer/index.html` on every element ID `renderer.js` looks up (`document.getElementById(...)`); a mismatch throws at load and breaks the whole page. Every handbook section that describes a concrete UI feature or workflow must include an example screenshot (`docs/images/`) — this is a standing requirement, not a one-off; purely textual/reference sections (privacy notices, FAQ, parameter tables) don't need one. Shared styles (`.btn-secondary`, `.icon-btn`, `.modal-overlay`, etc.) default to the blue Dozenten palette; when they're used inside Inventar-mode UI, add a `body.mode-inventar` override rather than leaving blue leaking through. Every handbook must also always be available as a PDF (`docs/handbuch.pdf`, `docs/handbuch-inventar.pdf`, `docs/admin-handbuch.pdf`) — regenerate all three with `npm run build:handbook-pdfs` (runs `docs/build-handbook-pdfs.js`, which uses Electron's `webContents.printToPDF` to render each handbook HTML file with full styling/images intact) whenever a handbook HTML file changes; `download.html`'s "PDF herunterladen" links point at these generated files, not the HTML. `docs/demo-video.webm` is a real (not staged/mocked) screen recording of the Inventar-only demo, captured with Playwright + the Chromium/ffmpeg binaries already present at `/opt/pw-browsers` (`recordVideo` on a `BrowserContext`) — Playwright itself is not a project dependency, only used ad hoc to (re)generate this asset; re-record it if the Inventar-Dashboard UI changes noticeably. The bundled ffmpeg build only has a VP8/WebM encoder (no H.264), so there is no MP4 alternative.
...
Each *Dozent* object is `{ id, name, todos, openProjects, doneProjects, chat }`. List items are `{ id, text, done }`; chat messages are `{ id, text, time }`. Each inventory item is `{ id, geraet, hersteller, zustand, stueckzahl, photo, ausgaben, nachbestellungen }`, where `zustand` is `'OVP'` or `'Gebraucht'`, `photo` is a captured `data:` URL or `null`, `ausgaben` is a log of `{ id, menge, empfaenger, datum }` issue-outs, and `nachbestellungen` is a log of `{ id, menge, status, datum }` reorder requests (`status`: `'offen'` | `'bestellt'` | `'erledigt'`). IDs come from the local `uid()` helper (timestamp + random). `MAX_DOZENTEN = 4` caps the number of instructors; `LOW_STOCK_THRESHOLD` drives the low-stock reminder badge/toast.

### Admin mode

`state.adminPin` (plaintext, `null` until first set) gates an in-session-only `isAdmin` flag in `renderer.js` — it is **not** persisted, so every launch starts locked. The "🔒 Admin-Modus" button in the Inventar toolbar prompts to set a PIN on first use or unlock with the existing one; unlocking reveals an admin panel at the top of the Inventar-Dashboard with a static list of all download links and the `nachbestellungen` management UI (add/status-change/delete), all gated behind `if (!isAdmin) return;` guards in their handler functions. This is a lightweight local UI gate for accidental-change protection, not real multi-user authentication — there is no encryption or hashing of the PIN.
