...
A fourth item in this repository, `schulungen/bau-projektmanagement/`, is not an app but a content deliverable: a set of PDF training materials ("Teilnehmerhandbuch", "Dozentenhandbuch", "Schulungsfolien", and interactive/solution exam PDFs) for a 6-day "Bau- und Projektmanagement" course, generated from Node scripts (Playwright/Chromium for the handbooks and slides, `pdf-lib` for the interactive exam forms). See `schulungen/bau-projektmanagement/README.md`. It shares no dependencies, build config, or CI with the Dozenten Dashboard or the other projects above; its generator scripts (`schulungen/bau-projektmanagement/src/`) have their own `package.json` and are run from within that directory, not from the repo root.
