...
A fourth item, `it-world-fiverr-ad/`, is a static marketing landing page (plain `index.html` + `style.css` + `script.js`, no build step, no dependencies) advertising a "Elegante Webseite" Fiverr gig under the brand "IT-World" — navy/gold/white color scheme, 3D glossy card effects, soft scroll/hover animations, and a clickable example-site gallery with a lightbox. It is not part of any app and is opened directly as a static file (or served statically); it shares nothing with the other three projects.
