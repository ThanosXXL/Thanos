...
`demo/` holds a static, standalone HTML page (`demo/index.html`) with an audio player and download button for `demo/lounge-band.mp3` — a 30-second synthetically generated (not a real recording) lounge-style background loop for a demo video. It has no build step and is not packaged with any of the apps; open `demo/index.html` directly in a browser.
