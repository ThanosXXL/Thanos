...
A fourth item in this repository, `schulungen/bau-projektmanagement/`, is not an app but a content deliverable: a set of PDF training materials ("Kursbeschreibung", "Teilnehmerhandbuch", "Dozentenhandbuch", "Schulungsfolien", interactive/solution exam PDFs, and a fillable "Teilnehmerliste") for a 6-day "Bau & Projektmanagement" course, generated from Node scripts (Playwright/Chromium for the HTML-based documents, `pdf-lib` + `@pdf-lib/fontkit` for the interactive PDF forms). All documents share an embedded Nunito font and a set of self-drawn SVG-based icon illustrations per topic (no stock photos). See `schulungen/bau-projektmanagement/README.md`. It shares no dependencies, build config, or CI with the Dozenten Dashboard or the other projects above; its generator scripts (`schulungen/bau-projektmanagement/src/`) have their own `package.json` and are run from within that directory, not from the repo root. `schulungen/bau-projektmanagement/ANFORDERUNGEN.md` is the standing requirements record for this content package (verbatim original request plus every later addition) and states an explicit process rule from the client: **ask for confirmation before making any further code change to this package, and only proceed after they approve.** Treat that rule as binding for this directory until the client says otherwise, even across sessions.
