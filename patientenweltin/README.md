...
- **`main.js`** — erstellt das `BrowserWindow` und persistiert den gesamten State als JSON unter
  `app.getPath('userData')/patientenweltin-data.json` via die IPC-Handler `load-data`/`save-data`.
- **`preload.js`** — exponiert `window.patientenweltinAPI` (`loadData()`, `saveData(data)`) als
...
(`background-clip: text` + `-webkit-text-fill-color: transparent`) plus Emboss-Schatten. Für den
aktiven Tab liegt der Effekt bewusst auf einem inneren `<span class="tab-label">`, nicht auf dem
`<button>` selbst, damit das eigene `background`-Shorthand des Buttons (für die Tab-Hervorhebung)
das Text-Clipping nicht überschreibt — bei neuen Glanztext-Stellen dasselbe Muster verwenden, falls
das Element selbst schon einen Hintergrund per `background`-Shorthand setzt.
